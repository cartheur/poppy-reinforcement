            ...
            public virtual object perform_action(object action) {
                ...
                if (eligible_actions.Contains(action)) {
                    if (this.epsilon == 0 || !this.probabilistic_event(this.epsilon)) {
                        ...
                        Console.WriteLine("MathematicalActor: Executing policy-action " + action.ToString() + " from " + current_state.ToString() + " to " + next_state.ToString());
                        ...
                        Console.WriteLine("MathematicalActor: Executing epsilon-random action" + random_action.ToString() + " from " + current_state.ToString() + " to " + next_state.ToString());
                        this.mathematicalObserver.current_state = next_state;
                    }
                } else {
